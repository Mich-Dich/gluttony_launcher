...
namespace AT::util {
    ...
    //          This function is automaticly called by the AT::aplication on startup
    ...
    std::filesystem::path file_dialog(const std::string& title = "Open", const std::vector<std::pair<std::string, std::string>>& filters = default_filters, bool select_directory = false);
