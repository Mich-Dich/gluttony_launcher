...
namespace AT::math {
	...
	glm::vec3 get_forward_vector(const glm::vec3 direction) {
		...
		const f32 yaw   = direction.y;
		const f32 pitch = direction.x;
	...
	glm::vec3 get_right_vector(const glm::vec3 direction) {
		...
		glm::vec3 forward_dir = get_forward_vector(direction);
		return glm::normalize(glm::cross(world_up, forward_dir));
    }


    glm::vec3 get_up_vector(const glm::vec3 direction)          { return glm::normalize(glm::cross(get_right_vector(direction), direction)); }
